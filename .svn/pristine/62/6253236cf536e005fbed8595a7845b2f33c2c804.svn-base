...
public class TrankRegel extends RegelController {
	...
	public boolean pruefeAuswahlErlaubt(Quelle quelle, List<Ziel> ziel) {
		...
		if(quelle.alsSchatz() != null){
			...
			else if(Schatz.SPRUCHROLLE.equals(quelle.alsSchatz()) || Schatz.ZEPTERDERMACHT.equals(quelle.alsSchatz()) || Schatz.TALISMAN.equals(quelle.alsSchatz()) || Schatz.DIEBESWERKZEUG.equals(quelle.alsSchatz()) || Schatz.KOEPFERKLINGE.equals(quelle.alsSchatz())){
				quelleok = true;
			...
			if(!held.isUltimativeEingesetzt()){
				if((Heldtyp.KREUZRITTERINPALADININ.equals(held.getHeldtyp()) || Heldtyp.ARKANERSCHWERTMEISTERKAMPFMAGIER.equals(held.getHeldtyp())) && !held.isLevelUp()){
					quelleok = true;
				}
				Phase phase = masterController.getDungeonRoll().getAktuellerZustand().getAbenteuer().getPhase();
		...
		if(!zieleok){
			...
			if(!ziel.isEmpty()){
				...
				if(!nichtAufFriedhof && quelle.alsSchatz() != null ){
					zieleok = false;
				}
				...
				if((ziel.size()-offset) > anzahlTraenke || ( (ziel.size()-offset) == 0 && offset == 1)){
					zieleok = false;
				}
	...
	public void regelAusfuehren(Quelle quelle, List<Ziel> ziel) {
		...
		if(quelle.alsSchatz() != null){
			...
			gefaehrten.removeIf(g -> g == quelle.alsWuerfel());	
		}
		...
		for(Ziel aktuellesZiel : ziel){
			...
			if(quelle.alsSchatz() == null || !Schatz.TRANK.equals(quelle.alsSchatz())){
				entferneTrank();
			}
		}
	...
	private void entferneTrank() {
		List<Wuerfel> dungeon = masterController.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDungeon();
		for(int i = 0; i < dungeon.size();i++){
			if(SchwarzeWuerfelseite.TRANK.equals(dungeon.get(i).getSchwarzeWuerfelseite())){
				dungeon.remove(i);
				return;
			}
		}
