...
public class MonsterPhasenRegelTest {
	...
	public void setUp() throws Exception {
		...
		arkanter.alsHeld().setHeldtyp(Heldtyp.ARKANERSCHWERTMEISTERKAMPFMAGIER);
		...
		paladin.alsHeld().setHeldtyp(Heldtyp.KREUZRITTERINPALADININ);
			...
			for (List<Ziel> ziel : Ziels) {
				...
				for (Quelle h : helds) {
					...
					try {
						...
						mc.getDungeonRoll().getAktuellerZustand().getAbenteuer()
								.setGefaehrten(new ArrayList<Wuerfel>() {
									{
						...
						System.out.println(quelle.alsWuerfel().getWeisseWuerfelseite().toString() + " passt nicht zu :"
								+ ziel.get(0).alsWuerfel().getSchwarzeWuerfelseite().toString() + " and "
								+ ziel.get(6).alsWuerfel().getSchwarzeWuerfelseite().toString() + " als den Held "
								+ h.alsHeld().getHeldtyp().toString());
			...
			for (List<Ziel> ziel : Ziels) {
				...
				for (Quelle h : helds) {
					...
					try {
						...
						if (mpr.pruefeAuswahlErlaubt(quelle, ziel)) {

							mpr.regelAusfuehren(quelle, ziel);
							
							if (quelle.alsWuerfel().getWeisseWuerfelseite().equals(WeisseWuerfelseite.SPRUCHROLLE)
									&& !h.alsHeld().getHeldtyp().equals(Heldtyp.ADEPTINHEXE)) {
								assertEquals(
										mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getGefaehrten().size(),
										0);
								assertEquals(mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDungeon()
										.size()
										+ mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDrachen().size(),
										7);
								assertEquals(
										mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getFriedhof().size(),
										1);
								return;

							}
							assertEquals(
									mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getGefaehrten().size(), 0);
							assertEquals(mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDungeon().size(),
									0);
							assertEquals(mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getFriedhof().size(),
									1);
							assertEquals(mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDrachen().size(),
									0);
						...
						System.out.println(
								mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getGefaehrten().size() + ":"
										+ mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDungeon().size()
										+ ":"
										+ mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getFriedhof().size()
										+ ":"
										+ mc.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDrachen().size());

						System.out.println(quelle.alsWuerfel().getWeisseWuerfelseite().toString()
								+ " nicht  richtig aus gefuehrt bei ziel :"
								+ ziel.get(0).alsWuerfel().getSchwarzeWuerfelseite().toString() + " and "
								+ ziel.get(6).alsWuerfel().getSchwarzeWuerfelseite().toString() + " als den Held "
								+ h.alsHeld().getHeldtyp().toString());
