...
public class NeuWuerfeln extends RegelController {
	...
	public void regelAusfuehren(Quelle quelle, List<Ziel> ziel) {
		...
		for (Ziel zielWuerfel : ziel) {
			...
			if(veraenderung.getSchwarzeWuerfelseite()!=null) {
				if(veraenderung.getSchwarzeWuerfelseite().equals(SchwarzeWuerfelseite.DRACHENKOPF)) {
					masterController.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDungeon().remove(veraenderung);
					masterController.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDrachen().add(veraenderung);
				}
			}
		}
		...
		//ggf Phase "zurücksetzen"
		if(masterController.getDungeonRoll().getAktuellerZustand().getAbenteuer().getDungeon().stream().anyMatch(wuerfel -> wuerfel.isMonster())){
			masterController.getDungeonRoll().getAktuellerZustand().getAbenteuer().setPhase(Phase.KAMPFPHASE);
		}
